...
class CategoryViewTests(APITestCase):
    ...
    def test_retrieve(self):
        ...
        response = self.client.get('/api/category/?name='+CATEGORY_NAME_3)
        self.assertEqual(200, response.status_code)
        payloads = response.json()
        ...
        response = self.client.get('/api/category/?company='+COMPANY_NAME_2)
        self.assertEqual(200, response.status_code)
        payloads = response.json()
        self.assertTrue(len(payloads) == 1)      # 読みだした Category は一つか？
        ...
        # /api/category/?company={name} のチェック
        response = self.client.get('/api/category/?parent_category='+CATEGORY_NAME_2)
        self.assertEqual(200, response.status_code)
        payloads = response.json()
        self.assertTrue(len(payloads) == 1)      # 読みだした Category は一つか？
        self.check_category(payloads[0], self.category_3)
    ...
    def test_create(self):
        ...
        response = self.client.get('/api/category/?name='+CATEGORY_NAME_4)
        self.assertEqual(200, response.status_code)
        payloads = response.json()
        self.assertTrue(len(payloads) == 1)      # 読みだした Category は一つか？
    ...
    # 
    # PUT のテスト
    #   category_1 を上書き後、id を指定して読み出しチェックする
    #
    def test_update(self):
        before_created_at = self.category_1.created_at
        before_updated_at = self.category_1.updated_at
        ...
        """
        to_do
            created_at が変更されていないこと
            updated_at が新しくなっていること
        """
        # 変更内容のチェック
        # 比較データの作成
        category = Category(
